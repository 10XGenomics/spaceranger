...
def split(args):
    ...
    if args.similarity_type not in SIMILARITY_TYPES:
        martian.exit(
            "Unsupported similarity type: {}. Must be one of: {}".format(
                args.similarity_type, ",".join(SIMILARITY_TYPES)
            )
        )

    with LogPerf("load"):
        pca_mat = load_pca_from_h5(args.pca_h5)[0].transformed_pca_matrix
    ...
    given_num_neighbors = (
        ...
        else analysis_constants.GRAPHCLUST_NEIGHBORS_DEFAULT
    )
    given_neighbor_a = (
        args.neighbor_a
        if args.neighbor_a is not None
        else analysis_constants.GRAPHCLUST_NEIGHBOR_A_DEFAULT
    )
    given_neighbor_b = (
        args.neighbor_b
        if args.neighbor_b is not None
        else analysis_constants.GRAPHCLUST_NEIGHBOR_B_DEFAULT
    )
    ...
    use_neighbors = int(
        max(
            given_num_neighbors,
            np.round(given_neighbor_a + given_neighbor_b * np.log10(len(use_bcs))),
        )
    )
...
def join(args, outs, chunk_defs, chunk_outs):
    ...
    with LogPerf("merge_nn"):
        nn = cr_graphclust.merge_nearest_neighbors(
            [chunk.chunked_neighbors for chunk in chunk_outs], chunk_defs[0].total_rows
        )
    print(f"nn\tnn_nodes\t{nn.shape[0]:0.4f}")
    print(f"nn\tnn_links\t{nn.nnz:0.4f}")
    print(f"nn\tnn_density\t{cr_graphclust.matrix_density(nn):0.4f}")
    sys.stdout.flush()
    ...
    if args.similarity_type == "snn":
        ...
        with LogPerf("louvain"):
            cr_graphclust.run_louvain_weighted_clustering(
                matrix_bin, matrix_weights, louvain_out, seed=args.random_seed
            )
        ...
        with LogPerf("louvain"):
            cr_graphclust.run_louvain_unweighted_clustering(
                matrix_bin, louvain_out, seed=args.random_seed
            )
