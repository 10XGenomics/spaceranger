...
HIRES_VISIUM_HD_MAX_DIM_DEFAULT = 6000
HIRES_MAX_DIM_DICT = {
    PipelineMode(Product.VISIUM, SlideType.XL): 4000,
    PipelineMode(Product.CYT, SlideType.XL): 4000,
    PipelineMode(Product.CYT, SlideType.VISIUM_HD): HIRES_VISIUM_HD_MAX_DIM_DEFAULT,
}
LORES_MAX_DIM = 600
...
def is_production_slide(slide_sample_area_id: str) -> bool:
    ...
    """
    return any(
        slide_sample_area_id.upper().startswith(x)
        for x in VISIUM_PRODUCTION_SLIDE_PREFIXES
    )
...
def is_hd_slide(slide_sample_area_id: str) -> bool:
    ...
    """
    return any(
        slide_sample_area_id.upper().startswith(x) for x in VISIUM_HD_SLIDE_PREFIXES
    )
...
def get_cytassist_capture_area(slide_sample_area_id: str) -> str:
    ...
    capture_area = SLIDE_CAPTURE_AREA_TO_CYTASSIST_CAPTURE_AREA_MAP.get(
        area_id.upper(), None
    )
    if capture_area:
        return capture_area
...
def get_cytassist_images_from_extracted_tgz_folder(
        ...
        with open(run_info_name[0]) as f:
            for line in f:
                if line.startswith(RUN_INFO_LINE_PREFIX):
                    run_name = line.removeprefix(RUN_INFO_LINE_PREFIX).strip()
    ...
    if not possible_cytassist_images:
        possible_cytassist_images = [
            ...
            and x.split("_", maxsplit=NUMBER_CYTASSIST_TIFF_FIELD_SEPARATORS)[
                -1
            ].rsplit(".", maxsplit=1)[0]
        ...
        if not possible_cytassist_images:
            possible_cytassist_images = [
                ...
                and x.split("_")[-2]
                in CAPTURE_AREA_TO_MACHINE_SUFFIX.get(capture_area, [])
                and x.split("_")[-1].rsplit(".", maxsplit=1)[0]
                not in TAGS_OF_TIFS_TO_NOT_PICK
            ]
...
def get_galfile_path(barcode_whitelist: str) -> str:
    ...
    path_to_galfile = os.path.join(
        cr_constants.BARCODE_WHITELIST_PATH, barcode_whitelist + ".gal"
    )
