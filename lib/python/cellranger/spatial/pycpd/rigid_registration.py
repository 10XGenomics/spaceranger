...
from cellranger.spatial.pycpd.expectation_maximization_registration import (
...
class rigid_registration(expectation_maximization_registration):
    def __init__(
        self, R=None, t: np.ndarray | None = None, s: float | None = None, *args, **kwargs
    ):
        super().__init__(*args, **kwargs)
        if self.D not in (2, 3):
            message = (
                f"Rigid registration only supports 2D or 3D point clouds. Instead got {self.D}."
            )
        ...
        self.s: float = 1 if s is None else s
    ...
    def update_variance(self):
        ...
        self.MSE = (xPx - 2 * self.s * trAR + self.s * self.s * self.YPY) / (2 * self.sigma2)
        self.q = self.MSE + self.D * self.Np / 2 * np.log(self.sigma2)
