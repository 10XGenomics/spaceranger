...
use crate::{io_utils, BarcodeSegment, BarcodeSegmentContent, BcSegSeq};
...
/// Assume any unrecognized identifier is a (potentially user-provided) CMO ID or HASHTAG ID.
        ...
        let iter = match self {
            WhitelistSource::TxtFile { path, strand } => {
                ...
                match strand {
                    ...
                    ReqStrand::Reverse => Box::new(
                        iter.map_ok(|(k, v, id)| (k.revcomp(), v.map(|v| v.revcomp()), id)),
                    )
...
impl Whitelist {
    ...
    /// Note that for translation whitelists, this only matches against the
    /// "key" barcodes, not the translated "value" barcodes.
    fn exact_match_with_translation(&self, sequence: &BcSegSeq) -> Option<BcSegSeq> {
        match self {
            Whitelist::Plain(ref whitelist) => whitelist.contains(sequence).then_some(*sequence),
            Whitelist::Trans(ref whitelist) => whitelist.get(sequence).copied(),
            Whitelist::SpatialHd(ref whitelist) => {
