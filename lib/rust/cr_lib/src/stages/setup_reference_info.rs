...
pub struct SetupCountAnalysisStageInputs {
...
pub struct SetupCountAnalysisStageOutputs {
    /// ReferenceInfo built from reference_path or target_set
    pub reference_info: ReferenceInfo,
}

/// SetupReferenceInfo stage
pub struct SetupReferenceInfo;

#[make_mro(volatile = strict)]
impl MartianMain for SetupReferenceInfo {
    type StageInputs = SetupCountAnalysisStageInputs;
    type StageOutputs = SetupCountAnalysisStageOutputs;
    fn main(&self, args: Self::StageInputs, _rover: MartianRover) -> Result<Self::StageOutputs> {
        ...
        Ok(SetupCountAnalysisStageOutputs { reference_info })
    }
}
...
mod tests {
    ...
    fn test_get_reference_info_probe_and_ref() -> Result<()> {
        insta::assert_snapshot!(get_reference_info(
