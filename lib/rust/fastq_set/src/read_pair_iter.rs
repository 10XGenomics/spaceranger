...
pub enum FastqError {
    ...
    #[error("TXRNGR10010: Error opening FASTQ file: '{}'", file.display())]
    Open { file: PathBuf, source: io::Error },

    #[error("TXRNGR10011: IO error in FASTQ file: '{}', line: {line}", file.display())]
    Io {
        file: PathBuf,
        line: usize,
        source: io::Error,
...
impl<T> FileIoError<T> for Result<T, io::Error> {
    #[cold]
    fn open_err(self, path: &Path) -> Result<T, FastqError> {
        match self {
            Ok(v) => Ok(v),
                    ...
                    if first_header != this_header {
                        ...
                        return Err(FastqError::format(
                            format!(
                                ...
                                this_path.display(),
                                first_header
                                    .map_or_else(String::new, |s| String::from_utf8_lossy(s)
                                        .to_string()),
                                this_header
                                    .map_or_else(String::new, |s| String::from_utf8_lossy(s)
                                        .to_string()),
...
mod test_read_pair_iter {
    ...
    #[cfg(target_os = "linux")]
    fn test_mem_single(every: usize, storage: ReadPairStorage) -> anyhow::Result<u64> {
