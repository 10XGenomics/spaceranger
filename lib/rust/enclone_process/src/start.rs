...
use crate::core::defs::{AltRef, CloneInfo, EncloneControl};
use crate::core::enclone_structs::{BarcodeFates, CandidateClonotype, EncloneExacts};
use crate::core::hcomp::heavy_complexity;
use crate::core::join_one::COMP_FILT;
use crate::disintegrate::{disintegrate_onesies, DisintegrateOnesiesResult};
use crate::doublets::delete_doublets;
use crate::enclone::allele::{find_alleles, sub_alts};
use crate::enclone::graph_filter::GraphFilter;
use crate::enclone::info::build_info;
use crate::enclone::join::join_exacts;
use crate::enclone::misc1::{ArtificialFoursieFilter, CrossFilter};
use crate::enclone::misc2::find_exact_subclonotypes;
use crate::enclone::misc3::sort_tig_bc;
use crate::enclone::BarcodeFilter;
...
use crate::process::loupe::make_donor_refs;
use crate::read_json::{parse_json_annotations_files, Annotations};
...
pub fn load_input_data(ctl: &EncloneControl, refdata: &RefData) -> anyhow::Result<Annotations> {
    ...
    flag_defective(ctl, refdata, &mut broken);
...
pub fn main_enclone_start(
    ...
    if !ctl.trace_barcode.is_empty() {
        for ex in &exact_clonotypes {
            for clone in &ex.clones {
                if clone[0].barcode == ctl.trace_barcode {
                    println!(
    ...
    let alt_refs = find_alleles(refdata, &exact_clonotypes);
    ...
    sub_alts(refdata, &alt_refs, &mut info[..], &mut exact_clonotypes[..]);
    ...
    if COMP_FILT < 1_000_000 {
        let jun = heavy_complexity(refdata, &exact_clonotypes, &drefs);
    ...
    if ctl.is_bcr() && (ctl.cr_opt.filter.umi_count || ctl.cr_opt.filter.umi_ratio) {
