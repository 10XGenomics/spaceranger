...
use crate::core::defs::{ExactClonotype, TigData};
use crate::Dataset;
...
pub struct CrossFilter<'a> {
    pub origin_info: &'a [Dataset],
}

impl BarcodeFilter<Vec<TigData>> for CrossFilter<'_> {
    fn fate_keys(&self, item: &Vec<TigData>) -> impl Iterator<Item = (usize, String)> {
        std::iter::once((item[0].dataset_index, item[0].barcode.clone()))
    }

    fn filter(&self, tig_bc: &[Vec<TigData>]) -> Vec<Option<BarcodeFate>> {
...
pub struct ArtificialFoursieFilter;
