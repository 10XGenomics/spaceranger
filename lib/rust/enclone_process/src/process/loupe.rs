...
use crate::core::defs::{AltRef, ColInfo, EncloneControl, ExactClonotype};
    ...
    for j in 0..exacts.len() {
        ...
        for cx in 0..cols {
            ...
            for p in 0..z {
                ...
                if p < vref.len() - REF_V_TRIM {
                    denom += 1;
                    if b != vref[p] {
                        diffs += 1;
                    }
                }
                if p >= z - (jref.len() - REF_J_TRIM) {
                    denom += 1;
            ...
            for p in 0..aa_seq.len() {
                ...
                if 3 * p + 3 <= vref.len() - REF_V_TRIM {
                    denom += 1;
                    if aa_seq[p] != codon_to_aa(&vref[3 * p..3 * p + 3]) {
                        diffs += 1;
                    }
                }
                if 3 * p > z - (jref.len() - REF_J_TRIM) + 3 {
                    denom += 1;
                    if aa_seq[p]
                        != codon_to_aa(
