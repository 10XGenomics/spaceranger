...
use metric::{Histogram, JsonReporter, MeanMetric, PercentMetric, SimpleHistogram, TxHashMap};
...
fn len_weighted_mean(v: &[i32]) -> f64 {
    let mut sum1 = 0_f64;
    let mut sum2 = 0_f64;
    for x in v {
        sum1 += f64::from(*x);
...
pub fn analyze_barcode_data(
    ...
    println!(
        "{:<6.1} = mean pairs per nonsolo umi",
        ucounts_all
            .iter()
            .map(|&x| x as i64)
            .collect::<MeanMetric>()
    ...
    println!(
        "{:<6.1} = N50 pairs per nonsolo umi",
        stats::n50(&ucounts_all).unwrap_or(0)
    );
    println!(
        "{:<6.1} = N90 pairs per nonsolo umi",
        stats::n90(&ucounts_all).unwrap_or(0)
    );
    println!(
