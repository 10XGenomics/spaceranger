...
#[derive(Serialize, Deserialize, Debug, PartialEq, Eq, Clone, Copy, strum_macros::Display)]
pub enum MetricTier {
...
impl MetricConditions {
    ...
    fn matches_section(&self, this_section: Section) -> bool {
        ...
        self.sections.iter().any(|section| this_section == *section)
    }
}
...
impl<T: MakePretty + TryFromValue + Serialize> Transformer for PassThroughTransformer<T> {
    fn transform(
        ...
        let pretty_value = val
            .as_ref()
            .map_or_else(|| "---".to_string(), MakePretty::make_pretty);
        Ok(TransformResult {
...
impl Transformer for CountAndPercentTransformer {
    fn transform(
        ...
        let pretty_value = val.make_pretty();
...
fn process_metric_group(
    ...
    for (key, cfg) in &group.entry_info {
        ...
        if let Some(group_by_key) = &group.group_by_key {
            if group_by_key == key {
                let grouping_key = val.value.as_str().unwrap_or_else(|| panic!("invalid metric grouping key value for metric {key}; must be a string, got {}", val.value));
