...
use cr_h5::molecule_info::{
    bin_barcodes, open_for_modification, trim_barcodes, BarcodeIdxType, LibraryIdxType,
    MoleculeInfoIterator, MoleculeInfoReader, MoleculeInfoWriter,
};
...
impl MartianStage for MergeMolecules {
    ...
    fn split(
        ...
        // The number of barcodes per chunk.
        ...
        // The maximum number of barcodes is the sum of the barcodes in each chunk.
        // It's an overestimate, because barcodes with zero counts are trimmed.
        let num_barcodes_sum: usize = num_barcodes.iter().sum();
        let join_mem_gib = 5 + barcode_mem_gib(num_barcodes_sum);

        for (i, (n, mem_gib)) in zip(num_barcodes, &chunk_mem_gib).enumerate() {
            println!("chunk={i},num_barcodes={n},mem_gib={mem_gib}");
        }
        println!("chunk=join,num_barcodes={num_barcodes_sum},mem_gib={join_mem_gib}");
    ...
    fn main(
        ...
        let is_visium_hd = MoleculeInfoReader::is_visium_hd(&molecule_h5)?;
